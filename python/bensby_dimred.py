...
X        = np.c_[data['Fe_H'],data['O_Fe'],data['Na_Fe'],data['Mg_Fe'],
                 ...
                 data['Ba_Fe'],data['Age']]
Xerr     = np.mean(np.c_[data['e_Fe_H'],data['e_O_Fe'],data['e_Na_Fe'],data['e_Mg_Fe'],
                 data['e_Al_Fe'],data['e_Si_Fe'],data['e_Ca_Fe'],data['e_Ti_Fe'],
                 data['e_Cr_Fe'],data['e_Ni_Fe'],data['e_Zn_Fe'],data['e_Y_Fe'],
                 data['e_Ba_Fe'],0.5*(data['B_Age1']-data['b_Age'])], axis=0)
if not age:
    X = X[:,:-1]; Xerr = Xerr[:-1]
if kin:
    ...
    for i in [0]:
        ...
        for ii in range(5):
            for jj in range(4):
                ...
                f.add_subplot(ax)
                scat = plt.scatter(Y[:, 0], Y[:, 1], c=colors[4*ii+jj],
                            s=5, cmap=plt.cm.jet)
                if jj!=0:
        ...
        rec = np.rec.fromarrays((data['HIP'], Y[:, 0], Y[:, 1]),
                                dtype=[('ID','|S18'),('X_tsne','float'),
                                       ('Y_tsne','float')])
        add = ""
        if age or kin or not feh:
            add = add + "_with"
            if age:
                add = add + "age"
            if kin:
        ...
        np.savetxt("specialsets/bensby2014_tsne_results"+add+str(p)+
                           "_rand"+str(i)+".csv", rec, delimiter=',',
               fmt = ('%s, %.5e, %.5e'))
        plt.savefig("../tSNE/Bensby2014_tsne_plots"+add+str(p)+
