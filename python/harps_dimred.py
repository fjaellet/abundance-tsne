...
Xerr     = np.mean(np.c_[data['erfeh'],data['errCu'],data['errZn'],data['errSr'],
                 data['errY'],data['errZrII'],data['errBa'],data['errCe'],
                 data['errAl'],data['errMg'],data['errSi'],data['errCa'],
                 data['errTiI'],data['agestd']], axis=0)
if not age:
    X = X[:,:-1]; Xerr = Xerr[:-1]
if kin:
...
for p in [5, 15, 30, 40, 50, 75, 100, 120, 150]:#[
    for i in [0]:
        ...
        rec = np.rec.fromarrays((data['Star'], Y[:, 0], Y[:, 1]),
                                dtype=[('ID','|S18'),('X_tsne','float'),
                                       ('Y_tsne','float')])
        add = ""
        if age or kin or teffcut or not feh:
            add = add + "_with"
            if age:
                add = add + "age"
            if kin:
            ...
            if teffcut:
                add = add + "teffcut"
        print add        
        np.savetxt("specialsets/harps_tsne_results"+add+str(p)+
                           "_rand"+str(i)+".csv", rec, delimiter=',',
               fmt = ('%s, %.5e, %.5e'))
        plt.savefig("../tSNE/HARPS_tsne_plots"+add+str(p)+
